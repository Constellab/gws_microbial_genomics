...
# 👋 Welcome to GWS Microbial Genomics

```gws_microbial_genomics``` is a [Constellab](https://constellab.io) library (called bricks) developped by [Gencovery](https://gencovery.com/). GWS stands for Gencovery Web Services.
...
Gencovery brick for microbial genomics analysis, wrapping bacterial genome annotation and analysis pipelines as Constellab tasks:
- Build/update a Bakta annotation database, and annotate prokaryotic genomes with Bakta (structural and functional annotation, genome plots)
- Functional annotation of protein/gene sequences with eggNOG-mapper, including downloading the eggNOG database
- Run the Bactopia bacterial genomics pipeline end-to-end from FASTQ reads (quality control, assembly, annotation, antimicrobial resistance detection with AMRFinderPlus, sequence typing with MLST, sketching), including samplesheet/metadata generation and results exploration

## 📄 Documentation

📄 For `gws_microbial_genomics` brick documentation, click [here](https://constellab.community/bricks/gws_microbial_genomics/latest/doc/getting-started)
...
The `gws_microbial_genomics` brick requires the `gws_core` brick.
...
```gws_microbial_genomics``` is completely free and open-source and licensed under the [GNU Affero General Public License v3.0](https://www.gnu.org/licenses/agpl-3.0.en.html).
